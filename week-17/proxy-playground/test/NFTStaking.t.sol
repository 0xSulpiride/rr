...
contract NFTStakingTest is Test {
    ...
    function setUp() public {
        ...
        vm.startPrank(owner);
        nft = WhitelistDiscountNFT(Upgrades.deployUUPSProxy("WhitelistDiscountNFT.sol", nftData));
        bytes memory stakingData = abi.encodeWithSelector(
            NFTStaking.initialize.selector,
            owner,
            address(nft)
        );
        staking = NFTStaking(Upgrades.deployUUPSProxy("NFTStaking.sol", stakingData));
        vm.stopPrank();
